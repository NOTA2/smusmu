module.exports = function (passport) {
  ...
  route.get('/register/commu/reschoolId', (req, res, next) => {
    if (req.user) { //로그인 정보가 있을 때(세션이 유지가 되어 있을 때)
      //일반 학생 계정일 경우
      if (req.user.kakaoId) {
        if (req.user.token == 'true')
  ...
  route.post('/register/commu/reschoolId', (req, res) => {
    ...
    crypto.randomBytes(20, function (err, buffer) {
      ...
      hasher({
        ...
        if (grade == '1') { //임원 계정 생성의 경우
          ...
          LEFT JOIN assoUser ON asso.assoUserId=assoUser.id
          WHERE college=? AND token='true'`
          conn.query(sql, [req.body.college], (err, results) => {
            if (err) {
              console.log(err);
              res.status(500);
            } else {
              user.assoId = results[0].assoId
              sql = "INSERT INTO assoUser SET ?"
              conn.query(sql, user, (err, results) => {
                if (err) {
                  console.log(err);
